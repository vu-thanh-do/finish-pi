...
class ExcelReaderService {
  ...
  _validateFile() {
    ...
    if (path.isAbsolute(this.filePath)) {
      ...
      // Nếu là đường dẫn tương đối, thử nhiều cách xử lý khác nhau
      const possiblePaths = [
        // Tương đối từ thư mục gốc dự án
        path.resolve(this.filePath),
        
        // Tương đối từ thư mục hiện tại
        path.join(process.cwd(), this.filePath),
        
        // Tương đối từ thư mục mã nguồn
        path.join(__dirname, '..', this.filePath),
        
        // Tương đối từ src
        path.join(__dirname, '..', '..', this.filePath.replace(/^\.\//, '')),
        
        // Đường dẫn trực tiếp đến thư mục data
        path.join(__dirname, '..', 'data', path.basename(this.filePath))
      ];
      
      // Kiểm tra từng đường dẫn có thể và sử dụng đường dẫn đầu tiên tồn tại
      absolutePath = possiblePaths.find(p => fs.existsSync(p));
      
      if (!absolutePath) {
        console.error(`File không tồn tại tại bất kỳ đường dẫn nào được thử:`);
    ...
    console.log(`>> Đã tìm thấy file Excel tại: ${absolutePath}`);
