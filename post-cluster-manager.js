...
class PostClusterManager {
  constructor(options = {}) {
    ...
    console.log(`>> [PostCluster] Khởi tạo với ${this.numWorkers} CPUs, tối đa ${this.maxConcurrentTasks} tác vụ đồng thời`);
  }

  /**
   * Thực thi một danh sách các tác vụ đăng bài
   * @param {Array<Object>} tasks - Danh sách tác vụ đăng bài
   * @returns {Promise<Array>} - Kết quả của tất cả các tác vụ
   */
  async executeTasks(tasks) {
    if (!tasks || tasks.length === 0) {
      console.log(`>> [PostCluster] Không có tác vụ nào để thực thi`);
      return [];
    }
    ...
    console.log(`>> [PostCluster] Chuẩn bị thực thi ${tasks.length} tác vụ đăng bài song song với ${this.numWorkers} CPUs`);
    ...
    if (this.failedTasks.length > 0) {
      console.log(`>> [PostCluster] Thử lại ${this.failedTasks.length} tác vụ thất bại...`);
      ...
      if (retriableTasks.length > 0) {
        console.log(`>> [PostCluster] Thử lại ${retriableTasks.length} tác vụ lỗi mạng/proxy...`);
  ...
  async scheduleNext() {
    return new Promise((resolve) => {
      ...
      if (this.taskQueue.length === 0 && this.runningTasks === 0) {
        console.log(`>> [PostCluster] Đã hoàn thành tất cả tác vụ`);
        ...
        this.executeTaskInWorker(task)
          .then(result => {
            ...
            if (this.results.length % 5 === 0 || this.results.length === 1 || this.taskQueue.length === 0) {
              ...
              console.log(`>> [PostCluster] Đã hoàn thành ${this.results.length}/${this.results.length + this.taskQueue.length} tác vụ (${successCount} thành công, ${failCount} thất bại)`);
            }
            ...
            if (this.taskQueue.length > 0 || this.runningTasks > 0) {
              ...
              console.log(`>> [PostCluster] Đã hoàn thành ${this.results.length} tác vụ tổng cộng`);
              resolve(this.results);
            }
          })
          .catch(error => {
            console.error(`>> [PostCluster] Lỗi khi thực thi tác vụ:`, error);
            this.results.push({
              success: false,
  ...
  executeTaskInWorker(task) {
    return new Promise((resolve, reject) => {
      try {
        ...
        const workerPath = path.resolve(__dirname, 'post-worker.js');
        ...
        worker.on('error', (err) => {
          console.error(`>> [PostCluster] Lỗi worker:`, err);
          worker.terminate();
          reject(err);
        });
        ...
        worker.on('exit', (code) => {
          if (code !== 0) {
            console.error(`>> [PostCluster] Worker exited with code ${code}`);
        ...
        console.error(`>> [PostCluster] Lỗi khi tạo worker:`, error);
        reject(error);
      }
    });
  }
  ...
  cleanup() {
    ...
    console.log(`>> [PostCluster] Đã dọn dẹp tài nguyên`);
  }
}

module.exports = PostClusterManager; 